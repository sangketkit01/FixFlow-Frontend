...
const AdminManage = () => {
  ...
  // ฟังก์ชันดึงข้อมูลช่าทั้งหมด (ทั้ง technicians และ registrations)
  const fetchAllTechnicians = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${API_BASE_URL}/admin/technicians-all`,
        {
          withCredentials: true,
        }
      );

      const sortedTechs = response.data.sort((a, b) => {
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      ...
      setFeedbackMessage(
        `เปลี่ยนสถานะไม่สำเร็จ: ${err.response?.data?.message || err.message}`
      );
      setIsSuccess(false);
    } finally {
